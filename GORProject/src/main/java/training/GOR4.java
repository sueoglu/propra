package training;

import model.GORModel;
import utils.ProteinSeq;

import java.util.List;

public class GOR4 extends GORTrain {
    public GOR4(List<ProteinSeq> trainSet) {
        super(trainSet);
    }

    @Override
    public GORModel GORTrain() {
        trainedModel = new GORModel("gor4");

        int windowSize = config.getWINDOW_SIZE();
        String aminoAcids = config.getAMINOACIDS();

        // Step 1: Iterate over the training set
        for (ProteinSeq seq : trainSet) {
            String aaSeq = seq.getSequence();
            String ss = seq.getSecondaryStructure();
            int windowSizeHalf = windowSize/2;

            // Step 2: Iterate over pos i+8 to i-8 in seq mit seq length = i
            for (int posInSeq = windowSizeHalf; posInSeq < aaSeq.length() - windowSizeHalf; posInSeq++) {

                // Step 3: check middle pos of window for current secondary structure ( = i + 7)
                char curSS = ss.charAt(posInSeq);

                // Step 4: check middle pos of window for current aa
                char curMiddleAA = aaSeq.charAt(posInSeq);
                if (config.getAMINOACIDS().indexOf(curMiddleAA) == -1) {
                    continue;
                }

                // Step 5: PROCESS WINDOW
                // update aa counts in SSMatrix of the Model (SS = SS at pos 0) for all aa in window
                int posInWindowMatrix = 0;
                for (int windowPos = posInSeq - windowSizeHalf; windowPos <= posInSeq + windowSizeHalf; windowPos ++) {
                    char pairAA = aaSeq.charAt(windowPos);

                    int countingIndexMatrixPos = posInWindowMatrix+1;

                    for (int countingAAIndex = windowPos+1; countingAAIndex <= posInSeq + windowSizeHalf; countingAAIndex++) {
                        char countingAA = aaSeq.charAt(countingAAIndex);
                        trainedModel.updateSSMatrix(countingIndexMatrixPos, countingAA, curSS, curMiddleAA, pairAA, posInWindowMatrix);
                        countingIndexMatrixPos++;
                    }

                    trainedModel.updateSSMatrix(posInWindowMatrix, pairAA, curSS, curMiddleAA);
                    posInWindowMatrix++;
                }
            }
        }

        return trainedModel;
    }
}
